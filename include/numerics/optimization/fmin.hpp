#ifndef NUMERICS_OPTIMIZATION_FMIN_HPP
#define NUMERICS_OPTIMIZATION_FMIN_HPP
...
namespace numerics {
    namespace optimization {
        // minimizes f(x) with respect to x on the interval [a, b] using Brent's
        // algorithm for one dimensional optimization. The function f should be
        // continuous on (a, b) but does not need to be continuous or defined at x=a or
        // x=b since the method will not evaluated at the end points.
        // see:
        // Brent, R. P. (1973). Algorithms for minimization without derivatives.
        // Englewood Cliffs: Prentice-Hall.
        template <std::floating_point real, std::invocable<real> Func>
        real fminbnd(Func f, real a, real b, real tol=std::sqrt(std::numeric_limits<real>::epsilon()))
        {
            if (b <= a)
                throw std::invalid_argument("invalid interval; a must be less than b but (a=" + std::to_string(a) + ", b=" + std::to_string(b) + ").");
            if (tol <= 0)
                throw std::invalid_argument("fminbnd(): require tol (=" + std::to_string(tol) + ") > 0");
            
            tol *= std::max<real>(1.0, std::abs(a+b)/2);

            real x,u,v,w,fu,fv,fw,fx;
            real c,d,e,m,p,q,r;
            ...
            while (std::max<real>(x-a,b-x) > 2*tol) {
                m = (a+b)/2;
                p=0;q=0;r=0;
                ...
                if (std::abs(d) >= tol)
                    u = x+d;
                else if (d > 0) // we do not want to evaluate the function again within tol of x.
                    u = x+tol;
                else
                    u = x-tol;
        ...
        // minimizes f(x) with respect to x using an initial value x0 and initial step
        // size alpha. This method is a one dimensional specialization of the
        // Nelder-Mead method.
        // see:
        // Saša Singer and John Nelder (2009) Nelder-Mead algorithm. Scholarpedia,
        // 4(7):2928.
        template <std::floating_point real, std::invocable<real> Func>
        real fminsearch(Func f, real x0, real alpha, real tol=std::sqrt(std::numeric_limits<real>::epsilon()))
        {
            tol *= std::max<real>(std::abs(x0), 1.0);
            
            uint best, worst;
            const real R=1.0, E=2.0, Co=0.5, Ci=0.5;
            real xr, fr, xe, fe, xc, fc;
            real x[2], fx[2];
