...
namespace numerics {
    ...
    {
        template <class vec, class Func, class Grad, std::floating_point eT=typename vec::value_type>
        eT wolfe_step(const Func& f, const Grad& grad_f, const vec& x, const vec& p, eT c1=eT(1.0e-4f), eT c2=eT(0.9f)) {
            auto g = [&](eT a) -> eT
            {
                return f(x + a*a*p);
            };
            uint best, worst, n_iter=0;
            const eT R=1.0, E=2.0, Co=0.5, Ci=0.5;
            eT ar, fr, ae, fe, ac, fc;
            eT a[2], fa[2];
            ...
            while (true)
            {
                ...
                bool cond2 = std::abs(__vmath::dot_impl(p, grad_f(x + a[best]*p))) <= c2*std::abs(__vmath::dot_impl(p, grad_f(x)));
