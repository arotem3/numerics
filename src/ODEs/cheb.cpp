...
/* CHEB : constructs the Chebyshev spectral differentiation matrix.
...
/* CHEB : constructs the Chebyshev matrix on the unit interval [-1,1].
 * --- D : cheb matrix storage.
 * --- x : grid storage.
 * --- m : number of points on interval. */
void ODE::cheb(arma::mat& D, arma::vec& x, uint m) {
    cheb(D,x,-1,1,m);
}