...
/* AM1 : multivariate implicit Euler method O(k) for any explicit first order system of ODEs.
...
/* AM1 : one dimensional implicit Euler method O(k) for any explicit first order system of ODEs.
 * our equations are of the form u' = f(t,u).
 * --- f  : f(t,u) [t must be the first variable, u the second].
 * --- t  : vector to store t-values initialized at {t_initial, t_final}.
 * --- U  : initial value u(t0).
 * --- opts : solver options. */
