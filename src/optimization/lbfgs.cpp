 ...
 * --- S : s_i = x_i - x_(i-1) 
 * --- Y : y_i = f_i - f_(i-1) */
void numerics::lbfgs::lbfgs_update(arma::vec& p, numerics_private_utility::cyc_queue& S, numerics_private_utility::cyc_queue& Y) {
    int k = S.length();
...
void numerics::lbfgs::minimize(std::function<double(const arma::vec&)> f,
    ...
    do {
        ...
        lbfgs_update(p, S_historic, Y_historic);
