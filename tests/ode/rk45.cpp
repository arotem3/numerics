...
#include "numerics/ode/rk45.hpp"

using numerics::ode::rk45;
...
int main()
{
    ...
    { // double
        ...
        if (emax > 1e-4) {
            std::cout << "rk45 failed double precision accuracy test.\n";
            ++n_failed;
        } else
            ++n_passed;
        ...
        if (emax > 1e-4) {
            std::cout << "rk45 failed double precision interpolation test.\n";
            ++n_failed;
        } else
            ++n_passed;
    }
    ...
    { // complex double
        ...
        if (emax > 1e-4) {
            std::cout << "rk45 failed complex double precision accuracy test.\n";
            ++n_failed;
        } else
            ++n_passed;
        ...
        if (A and B)
            ...
            std::cout << "rk45 failed safe interpolation check\n";
            ++n_failed;
        }
        ...
        if (emax > 1e-4) {
            std::cout << "rk45 failed complex interpolation accuracy test\n";
            ++n_failed;
        } else
            ++n_passed;
    }
    ...
    { // armadillo fvec
        ...
        if (emax > 1e-3) {
            std::cout << "rk45 failed armadillo vec single precision accuracy test.\n";
            ++n_failed;
        } else
            ++n_passed;
        ...
        if (not check) {
            std::cout << "rk45 failed to compute solution at requested points in tspan.\n";
            ++n_failed;
        } else
            ++n_passed;
    ...
    { // valarray double
        ...
        if (emax > 1e-3) {
            std::cout << "rk45 failed valarray double precision accuracy test.\n";
            ++n_failed;
        } else
            ++n_passed;
    }

    std::cout << "rk45 passed " << n_passed << " / " << n_passed + n_failed << " tests.\n";
