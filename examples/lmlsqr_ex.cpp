...
int main() {
    ...
    std::cout << "We will try to perform a nonlinear fit of the form:" << std::endl
              ...
              << "we will use initial guess b_hat = [1.5, 1.8, 1, 2]  because it is close to our actual parameters" << std::endl << std::endl;
    
    arma::vec x = arma::linspace(0,2*M_PI,1000);
    arma::vec y_true = arma::sin(2*x) + 1.2*arma::sin(M_PI_2*x);
    arma::vec y = y_true + arma::randn(arma::size(x));

    arma::vec b = {1, 2, 1.2, M_PI_2};
    arma::vec b_hat = {1.5, 1.8, 1, 2};
    ...
    std::function<arma::mat(const arma::vec&)> J = [x](const arma::vec& b) -> arma::mat {
    ...
    graph.plot_xy(x1,y_true1,"exact model");
    graph.plot_xy(x1,y_hat1,"least squares model");
